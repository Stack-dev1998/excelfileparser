...
function App() {
  ...
  const [sqlStructure, setSqlStructure] = React.useState([]);
  const [sqlTableHeaders, setSqlTableHeaders] = React.useState([]);
  ...
  //get fields of table from mysql
  function getSqlStructure(tableName) {
    (async () => {
      try {
        const rawResponse = await fetch(`/get-structure/${tableName}`, {
          method: "GET",
        });
        const content = await rawResponse.json();
        console.log(content);
        setSqlStructure(content);
        console.log(sqlStructure);
      } catch (err) {
        console.log(err);
      }
  ...
  function updateHeader(e, i) {
    ...
    if (e["target"]["value"] != "") {
      setHeader({ ...headers, [i]: e["target"]["value"] });
      setSqlTableHeaders({ ...sqlTableHeaders, [i]: sqlStructure[i] });
    }
      ...
      <div className="card p-3" style={{ width: "40rem" }}>
        ...
        <div className="columns">
          <div className="d-flex ">
            <div id="sql_structure">
              {sqlStructure &&
                sqlStructure.map((item, i) => {
                  return <p key={i}>{item} :</p>;
                })}
            </div>
            <div id="excel_structure">
              {sqlStructure &&
                sqlStructure.map((column, i) => {
                  return (
                    <p key={i}>
                      <select
                        name={`${i}`}
                      ...
                      >
                        ...
                        {columns.map((column, j) => (
                          <option key={`${column}-${i}`} value={column}>
                            {column}
                          </option>
                        ))}
                      </select>
